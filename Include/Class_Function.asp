...
'根据当前ID值返回类别ID
'*****************************************************
Function GetClassID(Table,ID)
	Dim Rs
	Set Rs=Conn.ExeCute("Select ClassID From "&Table&" Where ID="&ID&"")
	If Not Rs.Eof or Rs.Bof then
...
'根据当前ClassID值返回父级ID
'*****************************************************
Function GetNavParent(Table,ClassID)
	Dim Rs
	Set Rs=Conn.ExeCute("Select NavParent From "&Table&" Where ID="&ClassID&"")
	If Not Rs.Eof or Rs.Bof then
	GetNavParent=Rs("NavParent")
	end if
	Rs.Close:Set Rs=Nothing
End Function
...
Function GetPageenNavPath(Table,ID)
	Set Rs=Conn.Execute("Select ID,User_EnNavTtile,User_NavParent From "&Table&" Where ID="&ID&"")
	If Not (Rs.Eof Or Rs.Bof) Then
		Str=" > <a href=""?ID="&Rs("ID")&""">" & Rs("User_EnNavTtile") &"</a>"
		Str=GetPageenNavPath(Table,Rs("User_NavParent")) & Str
	End if
	GetPageenNavPath=Str
End Function
'*****************************************************
'根据当前ID值返回完整路径（例：首页 > 国内新闻 > 浙江新闻）
'*****************************************************
Function GetNavPath2(Table,ID)
	...
	If Not (Rs.Eof Or Rs.Bof) Then
		...
		Str=GetNavPath2(Table,Rs("NavParent")) & Str
	End if
	GetNavPath2=Str
End Function
...
Function GetNewsTitle(ID,length)
	...
	If Not (Rs_News.Eof Or Rs_News.Bof) Then
		if len(Rs_News("NewsTitle"))>length then
		GetNewsTitle=left(Rs_News("NewsTitle"),length)&"..."
		else
		GetNewsTitle=Rs_News("NewsTitle")
		end if
	Else
		GetNewsTitle="信息不存在或已被删除"
	End If
	Rs_News.Close
	Set Rs_News=Nothing
End Function
...
Function GetGradeName(Table,ClassID)
	Set Rs_Nav=Conn.Execute("Select NavTitle From "&Table&" Where ID="&ClassID&"")
	If Not (Rs_Nav.Eof OR Rs_Nav.Bof) Then
		GetSubNavName=Rs_Nav("NavTitle")
	End If
...
Function GetSelectaa(Table,NavParent)
	Dim Rs
	Set Rs=Conn.Execute("Select ID,NavTitle,EnNavTitle,NavParent,NavLevel From "&Table&" Where NavLock=0 And NavParent="&NavParent&" Order By id asc")
	If Not Rs.Eof Then
	Do While Not Rs.Eof
		Response.Write("<div>")
		Response.Write(Prefix(Rs("NavLevel")))
		Response.Write(Rs("NavTitle")&"---"&Rs("ID"))
		Response.Write("</div>")& vbCrLf
		Call GetSelectaa(Table,Rs("ID"))
	Rs.MoveNext
	If Rs.Eof Then Exit Do '防止造成死循环
	Loop
	End If
...
'返回年级列表模式
'*****************************************************
Function GetSelectGrade(Table,NavParent)
	Dim Rs
	Set Rs=Conn.Execute("Select ID,NavTitle,EnNavTitle,NavParent,NavLevel From "&Table&" Where NavLock=0 And NavParent="&NavParent&" Order By NavOrder Asc")
	If Not Rs.Eof Then
	Do While Not Rs.Eof
...
'**************************************************
'函数名：GetPage2
'作  用：获取分页,有返回类别ID,带关键字
'参  数：ID------当前ID,CAndE 0表示中文，1表示英文
'返回值：字符串
'**************************************************
Function GetPage2(Strsql,TableName,PageSize,CAndE)
Dim Rs,Str
...
if CAndE=1 then
	...
	Response.Write("[Home]&nbsp;[Previous]")
	Else
	Response.Write("[<a href=?Page=1&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">Home</a>]&nbsp;[<a href=?Page=")
	Response.Write(Page-1)
	Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">Previous</a>]")
	End If
	If Rs.PageCount-Page<1 Then
	Response.Write("[Next]&nbsp;[Last]")
	Else
	Response.Write("[<a href=?Page=")
	Response.Write(Page+1)
	Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">Next</a>]&nbsp;[<a href=?Page=")
	Response.Write(Rs.PageCount)
	Response.Write("&ClassID"&Request("ClassID")&"&keyword="&Request("keyword")&">Last</a>]")
	End If
	Response.Write("<select style=""FONT-SIZE: 9pt; FONT-FAMILY: 宋体"" onChange=""location=this.options[this.selectedIndex].value"" size=""1"" name=""Menu_1"">")
	For Pagei=1 To Rs.PageCount
	if Cint(Pagei)=Cint(Page) Then
	Response.Write("<option value=?Page=")
	Response.Write(Pagei)
	Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&" selected=selected>")
	Response.Write(Pagei)
	Response.Write("</option>")
	Else
	Response.Write("<option value=?Page=")
	Response.Write(Pagei)
	Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">")
	Response.Write(Pagei)
	Response.Write("</option>")
	End If
	Next
	...
	if CAndE =0 then
		...
		Response.Write("【首页】【上一页】")
		Else
		Response.Write("【<a href=?Page=1&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">首页</a>】【<a href=?Page=")
		Response.Write(Page-1)
		Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">上一页</a>】")
		End If
		If Rs.PageCount-Page<1 Then
		Response.Write("【下一页】【末页】")
		Else
		Response.Write("【<a href=?Page=")
		Response.Write(Page+1)
		Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">下一页</a>】【<a href=?Page=")
		Response.Write(Rs.PageCount)
		Response.Write("&ClassID="&Request("ClassID")&"&keyword="&Request("keyword")&">末页</a>】")
		End If
		Response.Write("<select style=""FONT-SIZE: 9pt; FONT-FAMILY: 宋体"" onChange=""location=this.options[this.selectedIndex].value"" size=""1"" name=""Menu_1"">")
		For Pagei=1 To Rs.PageCount
		if Cint(Pagei)=Cint(Page) Then
...
Response.Write("<form name='form11' id='form11' method='post' action='"&url&"&action=Message' target='_self' onsubmit='return CheckGB()'>")
...
if CAndE = 0 then
	Response.Write("<td width='400' style='color:#C32027;font-weight:bold;'></td><td></td>")
	...
	Response.Write("<td width='400' align='left'><input type='text' name='UserName' id='UserName' />&nbsp;")
...
if Request("action")="Message" then
	...
	Set Rs=Nothing
	if CAndE = 0 then
		Response.Write("<script>alert('感谢您的留言！');location.href='"&url&"';</script>")
	else
		if CAndE = 1 then
			Response.Write("<script>alert('Thank you for your message！');location.href='"&url&"';</script>")
		end if
	end if
	Response.End()
end if
...
'返回值：字符串
'**************************************************
Function AddGuestbooknew(url,CAndE)
Response.Write("<style type='text/css'>")
Response.Write("#ConMan,#ProName,#ProNumber,#Address,#Phone,#Email,#GuestbookTitle,#Tel,#CompanyName,#Website,#ICQ,#Fax{ width:380px; height:20px; border:1px solid #DCDBDB; margin-left:5px; background:url(../images/txt_bg.jpg) repeat-x left top; line-height:20px;}")
Response.Write("#Content{ width:380px; height:150px; border:1px solid #DCDBDB; margin-left:5px;background:url(../images/txt_bg.jpg) repeat-x left top;}")
Response.Write("#UserName{ width:150px; height:20px; border:1px solid #DCDBDB; margin-left:5px;background:url(../images/txt_bg.jpg) repeat-x left top;}")
...
if Request("action")="Message" then
	...
	Set Rs=Nothing
	if CAndE = 0 then
		Response.Write("<script>alert('感谢您的留言！');location.href='guestbooknew.asp';</script>")
	else
		if CAndE = 1 then
			Response.Write("<script>alert('Thank you for your message！');location.href='guestbooknew.asp';</script>")
		end if
	end if
	Response.End()
end if
...
Function twScript(Content)
		Response.Write("<Script>alert('"&Content&"');history.go(-1);</Script>")
		Response.End()
		twScript=Content
End Function

function checkpermission(cur_per,str_per)
  	Dim MyArray
	checkpermission=false
	MyArray = Split(str_per,",")
	
	For i = Lbound(MyArray) to Ubound(MyArray)
	if(cstr(MyArray(i)) = cstr(cur_per)) then
...
Function GetUserInfo(UserName,UserFile)

Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select ["&UserFile&"] From UserReg where UserName ='"&UserName&"'"
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
GetUserInfo = Rss(UserFile)
end if
Rss.Close:Set Rss=Nothing
End Function

Function GetSetsExperInfo(SetExperID,SetExperFile)

Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select ["&SetExperFile&"] From SetsExperiments where ID ="&SetExperID&""
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
GetSetsExperInfo = Rss(SetExperFile)
end if
Rss.Close:Set Rss=Nothing
End Function

Function ISParentSorce(Table,ParentID,TecID)
Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select * From "&Table&" where TecCode ='"&TecID&"' and StuCode='"&ParentID&"'"
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
ISParentSorce = true
else
...
Function GetSorce(Table,CodeID)
Set Rss = Server.CreateObject("Adodb.RecordSet")
'Sqls = "Select sum(TecSorce) as totalsorce From "&Table&" where TecCode ='"&CodeID&"'"
Sqls = "Select * From "&Table&" where TecCode ='"&CodeID&"'"
Rss.Open Sqls,Conn,1,1
GetSorce = 0
if not (Rss.Eof or Rss.Bof) then
	do while not Rss.eof
		...
		GetSorce = GetSorce + temp
	Rss.MoveNext
	Loop
	Set Rssa = Server.CreateObject("Adodb.RecordSet")
	Sqlsa = "Select * From "&Table&" where TecCode ='"&CodeID&"'"
	Rssa.Open Sqlsa,Conn,1,1
	GetSorce = left(GetSorce/Rssa.RecordCount,5)
	Rssa.Close:Set Rssa=nothing
end if
Rss.Close:Set Rss=Nothing
End Function

Function GetZjkh(Table,CodeID,zjkh)
Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select count(*) as totalsorce From "&Table&" where TecSorce='"&zjkh&"' and TecCode ='"&CodeID&"'"
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
	GetZjkh = Rss("totalsorce")
else
	GetZjkh=0
end if
Rss.Close:Set Rss=Nothing
...
Function GetSorce1(Table,CodeID,ISZC)
Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select sum(TecSorce) as totalsorce From "&Table&" where TecCode ='"&CodeID&"' and ISxz='"&ISZC&"'"
Rss.Open Sqls,Conn,1,1
GetSorce1 = 0
if not (Rss.Eof or Rss.Bof) then
	Set Rssa = Server.CreateObject("Adodb.RecordSet")
	Sqlsa = "Select * From "&Table&" where TecCode ='"&CodeID&"' and ISxz='"&ISZC&"'"
	Rssa.Open Sqlsa,Conn,1,1
	GetSorce1 = left(Rss("totalsorce")/Rssa.RecordCount,5)
	Rssa.Close:Set Rssa=nothing
end if
Rss.Close:Set Rss=Nothing
End Function

Function GetStuName(Table,StuID)
Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select * From "&Table&" where ExperiType ='"&StuID&"'"
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
	GetStuName = Rss("ExperiTitle")
end if
Rss.Close:Set Rss=Nothing
End Function

Function GetTecName(Table,TecCode)
Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select * From "&Table&" where Code ='"&TecCode&"'"
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
	GetTecName = Rss("UserName")
end if
Rss.Close:Set Rss=Nothing
End Function

Function ISChecked(Table,CodeID)
Set Rss = Server.CreateObject("Adodb.RecordSet")
Sqls = "Select * From "&Table&" where StuCode ='"&CodeID&"'"
Rss.Open Sqls,Conn,1,1
if not (Rss.Eof or Rss.Bof) then
	ISChecked = true
else
