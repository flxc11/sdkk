...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From RoomClass Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update RoomClass Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update RoomClass Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From RoomClass Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update RoomClass Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update RoomClass Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			...
			Conn.Execute("Delete From RoomClass Where ID In ("&AryID(i)&GetAllChild("RoomClass",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Room_Class.asp">教室分类维护</a><%=GetNavPath("RoomClass",ID)%></td>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="Room_Class.asp">返回分类根目录列表</a>|<a href="Room_ClassAdd.asp?ID=<%=ID%>">添加教室分类</a>&nbsp;</td>
</tr>
<tr>
...
<th colspan="2" class="Right">类别名称<a href="Room_ClassAdd.asp?ID=<%=ID%>">[+]</a></th>
...
Sql="Select * From RoomClass Where NavParent="&ID&" Order By NavOrder Asc"
...
<%=Rs("NavTitle")%>
</a><a href="?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">(<%=GetSubNavCount("RoomClass",Rs("ID"))%>)</a></td>
...
End If
%></td>
<td><a href="Room_ClassEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u4fe1\u606f\u5206\u7c7b\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
