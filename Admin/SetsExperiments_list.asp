...
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From ctsy Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('实验记录删除成功');window.location.href='?Page="&Page&"';</script>")
	Response.End()
End Select
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Experiment_list.asp">成套实验记录</a>
</td>
...
if UserGroup=3 then
	Sql= Sql +" Where UserName = '"&UserName&"' Order By ID Desc"
else
...
<th width="31%" class="Right">成套实验名称</th>
<th width="27%" class="Right">实验类别</th>
<th width="12%" class="Right">状态</th>
<th width="13%" class="Right">申请人</th>
<th width="9%">管理操作</th>
</tr>
...
<td class="Right"><a href="SetsExperiments_edito.asp?ID=<%=Rs("ID")%>&SetExperID=<%=Rs("SetExperID")%>&Page=<%=Page%>"><%=GetSetsExperInfo(Rs("SetExperID"),"ExperiTitle")%></a></td>
<td class="Right">
<%
  if(GetSetsExperInfo(Rs("SetExperID"),"ExperiType")=1) then
...
<td class="Right">
<%
	if Rs("SetExperiStatus")=0 then
	Response.Write("准备中")
	elseif Rs("SetExperiStatus")=1 then
	Response.Write("已准备")
	elseif Rs("SetExperiStatus")=2 then
	Response.Write("已完成")
	end if
%>
</td>
<td class="Right"><%=GetUserInfo(Rs("UserName"),"RealName")%></td>
<td><% if Rs("SetExperiStatus")=0 then%><a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ClassID=<%=ClassID%>" onclick="if(!confirm('确定删除该信息吗?')) return false;">删除</a><% else %>已锁定<% end if%></td>
</tr>
<%
i=i+1
Rs.MoveNext
