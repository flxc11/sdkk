...
If Action="Save" Then
	...
	Sql="Select NavOrder From RoomClass Where NavParent="&ID&" Order By NavOrder Desc"
	...
	'获取导航条的深度值
	Set Rs=Server.CreateObject("Adodb.RecordSet")
	Sql="Select NavLevel From RoomClass Where ID="&ID&""
	Rs.Open Sql,Conn,1,1
	If Not (Rs.Eof Or Rs.Bof) Then
	...
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('\u4fe1\u606f\u5206\u7c7b\u4fe1\u606f\u6dfb\u52a0\u64cd\u4f5c\u6210\u529f\u3002');window.location.href='Room_Class.asp?ID="&ID&"';</script>")
	Response.End()
End If
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Room_Class.asp">信息分类维护</a> >> 添加教室分类</td>
...
<td colspan="2"><input type="submit" value="保 存" class="Button"> <input type="button" value="返 回" class="Button" onClick="window.location.href='Room_Class.asp'"></td>
</tr>
</table>
</form>
</td>
