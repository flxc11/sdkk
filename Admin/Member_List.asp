...
Case "IsLock"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Response.Write("123")
			Conn.Execute("Update UserReg1 Set flg=false Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
Case "UnLock"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Update UserReg1 Set flg=true Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From UserReg1 Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
<th width="14%" class="Right">姓名<a href="#" onclick="top.CreateNewTab('Member_Add.asp','Member_Add','添加会员')">[+]</a></th>
<th width="17%" class="Right">工号</th>
<th width="10%" class="Right">校区</th>
<th width="10%" class="Right">任教学科</th>
<th width="10%" class="Right">年级</th>
<th width="16%" class="Right">班级</th>
<th width="9%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From UserReg1 Order By ID Desc"
...
<td class="Right"><%=Rs("Code")%>&nbsp;</td>
<td class="Right"><%=Rs("Campus")%>&nbsp;</td>
<td class="Right"><%=Rs("Subject")%>&nbsp;</td>
<td class="Right"><%=Rs("Grade")%>&nbsp;</td>
<td class="Right"><%=GetSubNavName("Roomclass",Rs("Roomclass"))%>&nbsp;</td>
<td><a href="?Action=Reset&ID=<%=Rs("ID")%>&Page=<%=Page%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5bf9\u8be5\u7ba1\u7406\u8d26\u53f7\u8fdb\u884c\u5bc6\u7801\u91cd\u7f6e\u7684\u64cd\u4f5c\u5417\uff1f\u9ed8\u8ba4\u5bc6\u7801\uff1a\u0031\u0032\u0033\u0034\u0035\u0036')) return false;">重置</a>
</td>
</tr>
<%
i=i+1
...
<th colspan="5" style="font-weight:normal;text-align:right"><a href="javascript:Delete();" style="font-weight:normal">删除</a>&nbsp;┊&nbsp;共<%=Rs.PageCount%>页&nbsp;第<%=Page%>页&nbsp;<%=PageSize%>条/页&nbsp;共<%=Total%>条&nbsp;
<%if Page=1 then%>
首 页&nbsp;上一页&nbsp;
<%Else%>
