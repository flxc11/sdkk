...
If Action="Save" Then
	...
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('实验申请成功，请等待管理员审核！');window.location.href='Experiment_add.asp';</script>")
	Response.End()
End If
%>
...
function CheckForm()
{
	...
	$("input[name='instru_check']:checked").each(function(){
      strnum+=yqts * ($(this).parent().next().children("input").val())+",";
	})
