 ...
 */
GV.rootMenu =	[
                 <% if (checkpermission(UserGroup,"1")) then%>["系统维护","",0,"1","1",true],<% end if %>
				<% if (checkpermission(UserGroup,"1")) then%>["内容管理","",1,"1","1",false],<% end if %>
				<% if (checkpermission(UserGroup,"1,2")) then%>["仪器管理","",2,"1","1",false],<% end if %>
				<% if (checkpermission(UserGroup,"1,2,3")) then%>["实验管理","",3,"1","1",false]<% end if %>
				<% if (checkpermission(UserGroup,"1,2")) then%>,["用户管理","",4,"1","1",false]<% end if %>
				<!--<% if (checkpermission(UserGroup,"1,2,3")) then%>,["成套实验","",5,"1","1",false]<% end if %>-->
				<% if (checkpermission(UserGroup,"1,2")) then%>,["教室管理","",6,"1","1",false]<% end if %>
				//["在线招聘","",4,"1","1",false]
				//,["留言管理","",5,"1","1",false]
				//["合作伙伴","",8,"1","1",false]
...
GV.menus[2] =	[
				["添加仪器","CreateNewTab('Pro_ContentAdd.asp','Pro_ContentAdd','添加仪器')","2","2"],
				["上架仪器维护","CreateNewTab('Pro_ContentListIn.asp','Pro_ContentListIn','上架仪器维护')","2","2"],
				//["下架商品维护","CreateNewTab('Pro_ContentListOut.asp','Pro_ContentListOut','下架商品维护')","2","2"],
				["仪器类别维护","CreateNewTab('Pro_Class.asp','Pro_Class','仪器类别维护')","2","2"]
...
GV.menus[3] =	[
				<% if (checkpermission(UserGroup,"1,2,3")) then%>["实验申请","CreateNewTab('Experiment_app.asp','Experiment_app','实验申请')","2","2"],
				["实验记录","CreateNewTab('Experiment_List.asp','Experiment_List','实验记录')","2","2"]<%end if%>
				<% if (checkpermission(UserGroup,"1,2")) then%>
				,["栏目管理","CreateNewTab('Experiment_Class.asp','Experiment_Class','栏目管理')","2","2"],
				["添加实验","CreateNewTab('Experiment_Add.asp','Experiment_Add','添加实验')","2","2"],
				["实验列表","CreateNewTab('Experiment_List.asp','Experiment_List','实验列表')","2","2"]
				<%end if%>
				];
...
<% if (checkpermission(UserGroup,"1,2")) then%>
GV.menus[4]=	[
				["添加用户","CreateNewTab('Member_Add.asp','Member_Add','添加用户')","2","2"],
				["用户列表","CreateNewTab('Member_List.asp','Member_List','用户列表')","2","2"]
				];
<%end if%>
...
//GV.menus[5] =	[
				<% if (checkpermission(UserGroup,"1,2")) then%>
				//["成套实验添加","CreateNewTab('SetsExperiments.asp','SetExperiment_add','成套实验添加')","2","2"],<%end if%>
				//["成套实验申请","CreateNewTab('SetsExperiments_app.asp','SetsExperiments_app','成套实验申请')","2","2"],
				//["成套实验记录","CreateNewTab('SetsExperiments_list.asp','SetsExperiments_list','成套实验记录')","2","2"]
				//];


GV.menus[6]=	[
				["添加教室","CreateNewTab('Room_ClassAdd.asp','Room_ClassAdd','添加教室')","2","2"],
				["教室列表","CreateNewTab('Room_Class.asp','Room_Class','添加教室')","2","2"]
				];
//GV.menus[5]=	[
...
//				//["客房预订","CreateNewTab('Booking.asp','Booking','客房预订')","2","2"]
//				];
