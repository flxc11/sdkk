...
<th width="20%" class="Right">联系人姓名</th>
<th width="22%" class="Right">留言类别</th>
...
<a href="LookGuestBook.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>"><%
	if len(Trim(Rs("UserName")))>18 then
	Response.Write(left(Trim(Rs("UserName")),15)&"...")
	else
	Response.Write(Trim(Rs("UserName")))
	end if
%></a>
</td>
<td class="Right"><% if Rs("GuestbookClass")=1 then%>留言反馈<% else %>客户反馈<% end if %></td>
<td class="Right">
<%
