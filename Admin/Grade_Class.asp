...
<%
Call ISPopedom(UserName,"Room_Class")
...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From GradeClass Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update GradeClass Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update GradeClass Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From GradeClass Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update GradeClass Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update GradeClass Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From RoomInfo Where ClassID="&AryID(i))
			Conn.Execute("Delete From GradeClass Where ID In ("&AryID(i)&GetAllChild("GradeClass",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Room_Class.asp">教室分类维护</a><%=GetNavPath("GradeClass",ID)%></td>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="Room_Class.asp">返回分类根目录列表</a>|<a href="Grade_ClassAdd.asp?ID=<%=ID%>">添加教室分类</a>&nbsp;</td>
</tr>
<tr>
...
<td width="90%" valign="top">以下为所有教室分类的信息列表，点击“+”或“分类名称”继续添加分类。<br>
...
<th colspan="2" class="Right">类别名称<a href="Grade_ClassAdd.asp?ID=<%=ID%>">[+]</a></th>
...
Sql="Select * From GradeClass Where NavParent="&ID&" Order By NavOrder Asc"
...
<%=Rs("NavTitle")%>
</a><a href="?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">(<%=GetSubNavCount("GradeClass",Rs("ID"))%>)</a></td>
...
End If
%></td>
<td><a href="Grade_ClassEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u4fe1\u606f\u5206\u7c7b\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
...
function ChangeParent()
{
    ...
    if (confirm('\u786e\u5b9a\u8981\u66f4\u6539\u9009\u4e2d\u7684\u7c7b\u522b\u7684\u6240\u5c5e\u7236\u7c7b\u522b\u5417\uff1f')) {
        window.location.href = 'Room_ClassParent.asp?ID='+l;
    }
}
function Delete() {
    var l = GetAllChecked();
