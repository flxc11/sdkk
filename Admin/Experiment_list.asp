...
ExperiRoom=ReplaceBadChar(Trim(Request("ExperiRoom")))
If ExperiRoom="" Then ExperiRoom=0
...
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From setsExperiments Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('删除成功');window.location.href='?Page="&Page&"';</script>")
	Response.End()
End Select
%>
...
$(document).ready(function(){
	$("#ExperiRoom").val("<%=ExperiRoom%>");
	$("#ExperiRoom").change(function(){
		window.location.href='?ExperiRoom='+$("#ExperiRoom").val();
	});
});
</script>
</head>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Experiment_list.asp">实验记录</a>
</td>
...
<select id="ExperiRoom" name="ExperiRoom" style="width:150px;">
  <option value="0">|--所有班级列表</option>
...
<%
if ExperiRoom>0 then
Sql = "Select * From setsexperiments where ExperiRoom='"&ExperiRoom&"' order by ID desc"
else
Sql = "Select * From setsexperiments order by ID desc"
end if
...
<th width="31%" class="Right">姓名<a href="#" onclick="top.CreateNewTab('Experiment_add.asp','Experiment_add','添加实验记录')">[+]</a></th>
<th width="27%" class="Right">学号</th>
<th class="Right">班级</th>
<th width="6%" class="Right">&nbsp;</th>
<th width="9%">管理操作</th>
</tr>
...
<td class="Right"><a href="Experiment_Edit.asp?ID=<%=Rs("ID")%>&ClassID=<%=Rs("ExperiRoom")%>&Page=<%=Page%>"><%=Rs("ExperiTitle")%></a></td>
<td class="Right">
<%=Rs("ExperiType")%>
</td>
<td class="Right">
<%=GetSubNavName("RoomClass",Rs("ExperiRoom"))%>
</td>
<td class="Right">&nbsp;</td>
<td><a href="Experiment_Edit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ClassID=<%=Rs("ExperiRoom")%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ExperiRoom=<%=ExperiRoom%>" onclick="if(!confirm('确定删除该信息吗?')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
