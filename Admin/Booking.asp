...
<%
Call ISPopedom(UserName,"Booking")
...
Select Case Action
Case "UnPublic"
	Conn.Execute("Update bookroom Set flg=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update bookroom Set flg=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From GuestBook Where ID In ("&AryID(i)&GetAllChild("bookroom",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
<th width="20%" class="Right">预订人</th>
<th width="22%" class="Right"> 客房类型 </th>
<th width="15%" class="Right">邮箱</th>
<th width="13%" class="Right">联系电话</th>
<th width="8%" class="Right">处理情况</th>
...
Sql="Select * From bookroom Order By posttime desc"
...
<td class="Right">
<a href="lookbooking.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>"><%=Rs("bookname")%></a>
</td>
<td class="Right"><%=Rs("roomtype")%></td>
<td class="Right">
<%
if len(Trim(Rs("bookemail")))>0 then
	Response.Write(Rs("bookemail"))
else
	Response.Write("&nbsp;")
end if
%></td>
<td class="Right">
<%
if len(Trim(Rs("booktel")))>0 then
	Response.Write(Rs("booktel"))
else
	Response.Write("&nbsp;")
end if
%></td>
<td class="Right"><% if Rs("flg")=True then Response.Write("已处理") else Response.Write("<font color='red'>未处理</font>") end if %></td>
<td class="Right">&nbsp;</td>
<td class="Right"><input type="checkbox" name="ID" value="<%=Rs("ID")%>" /></td>
<td class="Right">&nbsp;<a href="lookbooking.asp?ID=<%=Rs("ID")%>&amp;Page=<%=Page%>&amp;ParentID=<%=ID%>">查看</a>┊<a href="?Action=Delete&amp;ID=<%=Rs("ID")%>&amp;Page=<%=Page%>&amp;ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u5bfc\u822a\u6761\u4fe1\u606f\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
